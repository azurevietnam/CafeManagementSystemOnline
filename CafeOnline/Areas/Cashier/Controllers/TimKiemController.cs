...
namespace CafeOnline.Areas.Cashier.Controllers
{
    public class TimKiemController : Controller
    {
        ...
        {
            List<string> lsten = db.MATHANGs.Where(n => n.TenMatHang.Contains(term) || n.MaMatHang.StartsWith(term)).Select(t => t.TenMatHang).ToList();
            return Json(lsten, JsonRequestBehavior.AllowGet);
        }
        public JsonResult DsTenNhanVien(string term)
        {
            List<string> lsten = db.NGUOIDUNGs.Where(n => n.HoTenNV.Contains(term) || n.TenDangNhap.Contains(term)).Select(t => t.HoTenNV).ToList();
            return Json(lsten, JsonRequestBehavior.AllowGet);
        }
    }
}